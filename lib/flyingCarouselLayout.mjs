...
export const SETTLED_SCALE_BOOST = 1.18;
...
export const REVEAL_DURATION = 0.085;
...
export function createFlyingCarouselLayout({
  ...
  const settledBoostProgress = clamp01((viewportPixelWidth - 900) / 300);
  const settledScale = fit * (
    1 + (SETTLED_SCALE_BOOST - 1) * settledBoostProgress
  );
  const xGap = (CARD_WIDTH + 0.12) * settledScale;
  const yGap = (CARD_HEIGHT + 0.12) * settledScale;
  ...
  return Array.from({ length: cardCount }, (_, index) => {
    ...
    const orbit = {
      // All cards first populate the shallow/far ribbon seen in the
      // recording, then each follows nearly one complete lap of the same
      // closed path.
      offset: -REAR_ARC * 0.5 + index * (REAR_ARC / (cardCount - 1)),
      spatialFit,
      scaleFit: fit,
    };
    const card = {
    ...
    return {
      ...
      scatter: flyby,
      target: {
        position: [(column - (columns - 1) / 2) * xGap, ((rows - 1) / 2 - row) * yGap, 0],
        rotation: [0, 0, 0],
        scale: settledScale,
      },
    };
